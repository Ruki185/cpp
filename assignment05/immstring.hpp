...
public:
    ...
    ImmutableString concat(ImmutableString other) const;
    ...
    // This function is a good candidate for being defined in the header (inline)
    size_t size() const {  //const because reasons, not really necessary, but use const whenever it's possible
      return m_size;
    }
...
private:
    ...
    ImmutableString(char* buf, size_t len);

    // Can you use `const` to restrict the usage of these private data members?
    // Why should you?
    size_t      m_size; // length of string/number of characters excluding `\x0`
